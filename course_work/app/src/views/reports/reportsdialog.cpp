...
void ReportsDialog::loadReport(ReportType type) {
    ...
    switch(type) {
        case StockBalances:
            ...
            queryStr = "SELECT category_name AS \"Категория\", product_id AS \"ID Товара\", "
                       "product_name AS \"Наименование\", retail_price AS \"Цена\", stock AS \"Остаток\" "
                       "FROM v_stock_balances";
            break;

        case EmployeeKPI:
            setWindowTitle("КПЭ Сотрудников (Аналитика)");
            queryStr = "SELECT employee_name AS \"ФИО Сотрудника\", orders_count AS \"Кол-во заказов\", "
        ...
        case OrderStatuses:
            ...
            queryStr = "SELECT order_id AS \"№ Заказа\", created_at AS \"Дата оформления\", "
    ...
    if (m_queryModel->lastError().isValid()) {
        QMessageBox::critical(this, "Ошибка построения отчета", Utils::formatSqlError(m_queryModel->lastError()));
    } else {
...
void ReportsDialog::calculateTotals(ReportType type) {
    ...
    if (type == EmployeeKPI) {
        ...
        for (int row = 0; row < m_queryModel->rowCount(); ++row) {
            ...
            totalRevenue += m_queryModel->data(m_queryModel->index(row, 2)).toDouble();
        }

        ui->lblSummary->setText(QString("ИТОГО: Заказов — %1 шт. | Общая выручка — %2 руб.")
                                .arg(totalOrders).arg(totalRevenue, 0, 'f', 2));
        ui->lblSummary->show();
    }
        ...
        ui->lblSummary->setText(QString("ИТОГО: Товарных единиц — %1 шт. | Стоимость склада — %2 руб.")
                                .arg(totalStock).arg(totalValue, 0, 'f', 2));
        ui->lblSummary->show();
    }
}
...
void ReportsDialog::buildSalesMatrix() {
    ...
    QSqlQuery qClients("SELECT client_name, SUM(total_spent) as spent "
    ...
    while (qClients.next()) {
        ...
        QString shortName = parts[0] + (parts.size() > 1 ? " " + parts[1].left(1) + "." : "");
    ...
    QSqlQuery qProducts("SELECT DISTINCT product_name FROM v_sales_matrix ORDER BY product_name");
    ...
    QSqlQuery qData("SELECT client_name, product_name, total_quantity FROM v_sales_matrix");
    while (qData.next()) {
        ...
        if (col != -1 && row != -1) {
            ...
            item->setTextAlignment(Qt::AlignCenter);
            item->setBackground(QBrush(QColor(230, 255, 230))); // Подсветка ячейки с продажей
