...
void UsersDialog::updateUsersTable() {
  m_usersModel->setQuery(
      "SELECT u.id, e.full_name AS \"Сотрудник\", u.login AS \"Логин\", u.role "
      "AS \"Роль\" "
...
void UsersDialog::on_btnSave_clicked() {
  if (ui->cbEmployees->currentIndex() == -1)
    return;
  ...
  if (!userExists && pass.isEmpty()) {
    QMessageBox::warning(this, "Внимание",
                         "Для новой учетной записи необходимо задать пароль!");
    return;
  }
  ...
  if (userExists) {
    if (pass.isEmpty()) {
      q.prepare("UPDATE app_users SET login = :login, role = CAST(:role AS "
                "user_role) WHERE employee_id = :eid");
    } else {
      q.prepare("UPDATE app_users SET login = :login, password_hash = :hash, "
                "role = CAST(:role AS user_role) WHERE employee_id = :eid");
  ...
  if (!q.exec()) {
    QMessageBox::critical(this, "Ошибка БД",
                          Utils::formatSqlError(q.lastError()));
  } else {
...
void UsersDialog::on_btnDelete_clicked() {
  ...
  if (row < 0) {
    QMessageBox::warning(this, "Внимание",
                         "Выберите пользователя для удаления доступа!");
    return;
  }
...
void UsersDialog::on_tvUsers_clicked(const QModelIndex &index) {
  int row = index.row();
  ui->cbEmployees->setCurrentText(
      m_usersModel->record(row).value("Сотрудник").toString());
  ui->leLogin->setText(m_usersModel->record(row).value("Логин").toString());
  ui->cbRole->setCurrentText(
      m_usersModel->record(row).value("Роль").toString());
  ui->lePassword->clear();
}
