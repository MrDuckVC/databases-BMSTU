...
13. Вывести список таблиц и их структуру.
...
В unix системах кластер расположен по адресу `/var/lib/postgres/data`
...
* **Indexes (Индексы)**. Отдельная структура данных, связанная с конкретными столбцами таблиц. Может содержать копии данных из индексируемых столбцов или ссылки на них. Позволяет существенно повысить скорость выборки данных. Индексы не имеют смысла без таблиц. Индексирование может быть выполнено по одному или нескольким столбцам и произведено в любой момент.

* **Keys (Ключи)**. Ключ – это столбец или группа столбцов в таблице. Не существуют отдельно от таблиц.
...
* **Triggers (Триггеры)**. Триггер - специальный вид хранимой процедуры. Триггер может выполняться при изменениях данных или событиях в базе данных.
...
Консольная утилита для работы с Postgres, подробнее [здесь](https://postgrespro.ru/docs/postgresql/9.5/app-psql).
...
*Прим.* Флаг –u с аргументом postgres означает, что команда выполняется под
пользователем с именем postgres (пользователь по умолчанию в PostgreSQL).
...
```sql
CREATE TABLE t1(field1 integer, field2 char(10));
```
...
Чтобы создать именованное ограничение, напишите ключевое слово CONSTRAINT, а за ним идентификатор и собственно определение ограничения. (Если вы не определите имя ограничения таким образом, система пропишет имя за вас.)
...
Таблица может иметь только один первичный ключ. (Ограничений уникальности и ограничений NOT NULL, может быть сколько угодно.)
...
* Для того, чтобы для поля order_id применялось автоинкрементирование, нужно указать GENERATED ALWAYS AS IDENTITY при этом создается последовательность из integer с автоинкрементом. Ограничение NOT NULL указывает, что ячейке столбца нельзя присваивать значение NULL.
