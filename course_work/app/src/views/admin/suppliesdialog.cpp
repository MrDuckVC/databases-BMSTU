...
SuppliesDialog::SuppliesDialog(QWidget *parent)
  ...
  m_supplyModel->setHorizontalHeaderLabels(
      {"ID Товара", "Наименование", "Кол-во"});
...
SuppliesDialog::~SuppliesDialog() { delete ui; }
...
void SuppliesDialog::on_btnAddItem_clicked() {
  ...
  row << new QStandardItem(QString::number(productId))
      << new QStandardItem(name) << new QStandardItem(QString::number(qty));
...
void SuppliesDialog::on_btnProcess_clicked() {
  if (m_supplyModel->rowCount() == 0 || ui->cbSuppliers->currentIndex() == -1) {
    QMessageBox::warning(this, "Ошибка",
                         "Проверьте выбор поставщика и список товаров!");
    return;
  }
  ...
  q.prepare("INSERT INTO supply_items (supply_id, product_id, quantity) VALUES "
            "(:sid, :pid, :qty)");
  for (int i = 0; i < m_supplyModel->rowCount(); ++i) {
  ...
  if (db.commit()) {
    QMessageBox::information(this, "Успех",
                             "Поставка проведена. Склад обновлен.");
    accept();
  } else {
    db.rollback();
    QMessageBox::critical(this, "Ошибка",
