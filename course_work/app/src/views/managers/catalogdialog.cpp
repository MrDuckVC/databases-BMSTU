...
CatalogDialog::~CatalogDialog() { delete ui; }

void CatalogDialog::loadData(const QString &filter) {
  // Используем VIEW v_stock_balances для получения актуальных остатков и
  // категорий
  QString queryStr =
      "SELECT category_name AS \"Категория\", product_id AS \"ID Товара\", "
      "product_name AS \"Наименование\", retail_price AS \"Цена (руб)\", stock "
      "AS \"Остаток\" "
