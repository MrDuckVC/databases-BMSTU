...
OrdersDialog::~OrdersDialog() { delete ui; }
...
void OrdersDialog::loadProducts() {
  ui->cbProducts->clear();
  QSqlQuery q("SELECT id, product_name, retail_price, stock FROM products "
              "WHERE stock > 0");
  while (q.next()) {
...
void OrdersDialog::on_btnAddItem_clicked() {
  if (ui->cbProducts->currentIndex() == -1)
    return;
  ...
  for (int i = 0; i < m_cartModel->rowCount(); ++i) {
    if (m_cartModel->item(i, 0)->text() == QString::number(productId)) {
      QMessageBox::warning(this, "Внимание",
                           "Товар уже в корзине. Обновите позицию, если нужно "
                           "изменить количество.");
      return;
    }
  }
...
void OrdersDialog::on_btnRemoveItem_clicked() {
  ...
  if (row >= 0) {
    ...
    QMessageBox::warning(this, "Внимание",
                         "Выберите товар для удаления из корзины.");
  }
}
...
void OrdersDialog::on_btnCreateOrder_clicked() {
  if (m_cartModel->rowCount() == 0 || ui->leAddress->text().isEmpty()) {
    QMessageBox::warning(this, "Внимание",
                         "Заполните адрес и добавьте товары в корзину.");
    return;
  }
  ...
  if (!query.exec() || !query.next()) {
    ...
    QMessageBox::critical(this, "Ошибка",
                          Utils::formatSqlError(query.lastError()));
    return;
  }
  ...
  if (res == QMessageBox::Yes) {
    ...
    QString summary =
        QString("Клиент: %1<br>Адрес: %2<br><br><b>%3</b>")
            .arg(clientName, ui->leAddress->text(), ui->lblTotal->text());

    ReportExporter::exportToPdf(QString("Товарный чек №%1").arg(newOrderId),
                                summary, m_cartModel);
  }
