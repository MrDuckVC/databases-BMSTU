...
ChartViewDialog::ChartViewDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::ChartViewDialog) {
  ui->setupUi(this);
...
ChartViewDialog::~ChartViewDialog() { delete ui; }
...
void ChartViewDialog::setupChart() {
  ...
  QSqlQuery q(
      "SELECT employee_name, total_revenue FROM v_employee_kpi LIMIT 10");
  while (q.next()) {
  ...
  if (categories.isEmpty()) {
    QMessageBox::information(this, "Аналитика",
                             "Нет данных о продажах для построения диаграммы.");
    return;
  }
...
void ChartViewDialog::on_btnPrint_clicked() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, "Сохранить график", "Диаграмма_выручки.pdf", "PDF Files (*.pdf)");
  if (fileName.isEmpty())
    return;
